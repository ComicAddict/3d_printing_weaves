...
class EXPORTER_PT_Panel(bpy.types.Panel):
...
#        except: return False
    ...
    def draw(self, context): 
        ...
        row.prop(exporter, 'subdivide')

        sub = row.row()
        sub.prop(exporter, "max_segment_size")  
        sub.enabled =  exporter.subdivide #sub is not grayed out when 'linesegmentatio/subdivide/ bool is True
        ...
        col.prop(exporter, "nozzle_diameter", text='Nozzle Size')
        col.prop(exporter, "travel_speed", text='Travel Speed')
        col.prop(exporter, "extrusion_speed", text='Extrusion Speed')
        ...
        row.label(text="max:") 
        row = col.row(align=True)
        row.prop_search(exporter, "flow_map", context.active_object.data, "vertex_colors", text="")
        row.separator()
        row.prop(exporter, "min_flow")
        row.prop(exporter, "max_flow")

        col.separator()
        col.label(text="Speed Multiplier:") 
        row = col.row(align=True)
        row.prop_search(exporter, "speed_map", context.active_object.data, "vertex_colors", text="")    
        row.separator()   
        row.prop(exporter, "min_speed")
        row.prop(exporter, "max_speed")
     
        col.separator(factor=2)
    
        #col.label(text="Tool change based on color:") 
        col.prop(exporter, 'tool_color')
       
        
        row = col.row(align=True)
        row.enabled = exporter.tool_color
        row.prop(exporter, "color_T0", text="")
        row.prop_search(exporter, "T0", bpy.data, "texts", text="")   
        row.separator(factor=2)   
        row.prop(exporter, "color_T1", text="")
        row.prop_search(exporter, "T1", bpy.data, "texts", text="")   
        ...
        row.prop_search(exporter, "start", bpy.data, "texts", text="")   
        row=col.row(align=True)
        row.label(text='End G-code:')
        row.prop_search(exporter, "end", bpy.data, "texts", text="")   
...
def import_gcode(context, filepath):
        ...
        if exporter.subdivide:
            model.subdivide(exporter.max_segment_size)
        model.classifySegments()
        if exporter.split_layers:
...
def export_gcode(context):
    ...
    nozzle_diameter = exporter.nozzle_diameter
    extrusion_speed = exporter.extrusion_speed
    travel_speed = exporter.travel_speed
    ...
    #calcE
            ...
            multiplier = remap(multiplier, exporter.min_flow, exporter.max_flow)
...
def register():
    bpy.utils.register_class(exporter_PT_Panel)
...
def unregister():
    bpy.utils.unregister_class(exporter_PT_Panel)
